    ...
    return (
        <form className={`${styles["loginWrapper"]} d-flex flex-column justify-content-center p-5`}>
            ...
            <div className={`d-flex flex-column ${styles["formInputs"]}`}>
                <CustomInput type={"text"} hint={"Username"} onChangeHandler={emailHandler}/>
                <CustomInput type={"password"} hint={"Password"} onChangeHandler={passwordHandler}/>
            </div>
            { showError && 
                <div className="alert-danger alert mt-3 text-center">
                    {errorMsg}
                </div>
            }
            <div className="d-flex flex-row justify-content-center mt-5">
                <CustomButton title={"Login"} styleClass={"buttonPrimary"} handler={loginHandler}/>
            </div>
        </form>
    );
}